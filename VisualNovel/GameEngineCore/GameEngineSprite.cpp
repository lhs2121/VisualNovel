...
void GameEngineSprite::ResCreateFolder(std::string_view _Path)
{
	...
	std::vector<GameEngineFile> pngFiles;

	for (size_t i = 0; i < Files.size(); i++)
	{
		std::string EXT = Files[i].GetExtension();
		if (EXT == ".png")
		{
			pngFiles.push_back(Files[i]);
		}
	}

	SpriteDatas.resize(pngFiles.size());

	for (size_t i = 0; i < pngFiles.size(); i++)
	{
		GameEngineFile& File = pngFiles[i];

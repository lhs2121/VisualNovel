...
public:
	...
	template<typename LevelType>
	static void CreateLevel(const int _Name)
	{
		std::string Name = std::to_string(_Name);

		// 이미 내부에 TitleLevel이 존재한다.
		if (AllLevel.end() != AllLevel.find(Name))
		{
			MsgBoxAssert(Name + "의 이름을 가진 GameEngineLevel은 이미 존재합니다.");
			return;
		}

		std::shared_ptr<GameEngineLevel> NewLevel = std::make_shared<LevelType>();
		LevelInit(NewLevel, Name);
		AllLevel.insert(std::make_pair(Name, NewLevel));
	}
